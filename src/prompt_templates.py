...
def create_classification_template(
    ...
    reasoning_section = (
        "\nProvide your reasoning before giving the final answer."
...
def create_extraction_template(
    fields: list[str], task_description: str = "Extract information"
) -> str:
...
def create_summarization_template(
    max_length: int | None = None, style: str = "concise", focus: str | None = None
) -> str:
    ...
    """
    length_instruction = (
        f"Keep the summary to no more than {max_length} sentences."
        if max_length
        else ""
    )
...
def create_translation_template(
    target_language: str, preserve_formatting: bool = True
) -> str:
...
def create_code_generation_template(
    language: str, include_comments: bool = True, handle_errors: bool = True
) -> str:
    ...
    """
    comment_instruction = (
        "Include clear comments explaining the code." if include_comments else ""
    )
...
def create_question_answering_template(
    context_key: str = "context", answer_format: str = "direct"
) -> str:
    ...
    format_instruction = format_instructions.get(
        answer_format, format_instructions["direct"]
    )
